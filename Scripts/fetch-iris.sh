#!/usr/bin/env bash
#
# Fetch, verify and compile the pinned iris landmark Core ML model.
#
...
MODELS_DIR="Models"
MLMODEL_FILE="iris_landmark_64x64_float32.mlmodel"
FINAL_DIR="$MODELS_DIR/iris"
COMPILED_NAME="iris_landmark_64x64_float32.mlmodelc"
...
LOCK_DIR="$MODELS_DIR/.iris-install.lock"

SOURCE_URL="https://s3.ap-northeast-2.wasabisys.com/pinto-model-zoo/049_iris_landmark/resources.tar.gz"
SOURCE_TAR_SHA256="855a639e9ec9718dffb9d5fef857698b56b537b0e8a8746ce80ef4577a5fffe0"
INNER_TAR_SHA256="b48393f7fae66accebd642919836e6e3682039b831dd6b74acaf11838fa1686a"
MLMODEL_SHA256="a121e7b2afda92ba0f5f370ef49e15b9eecb7376e1e822a21d2778c674af78bb"
INNER_MEMBER="08_coreml/resources.tar.gz"
...
acquire_lock() {
  ...
  if [ -n "$pid" ] && ! kill -0 "$pid" 2>/dev/null; then
    echo "error: stale iris install lock $LOCK_DIR (owner $owner is not running)" >&2
  elif [ -n "$owner" ]; then
    echo "error: another iris install is in progress (lock $LOCK_DIR, owner $owner)" >&2
  else
...
SCRATCH="$(mktemp -d "${TMPDIR:-/tmp}/smart-gaze-iris.XXXXXX")"

echo "Downloading pinned source archive..."
curl --fail --location --silent --show-error \
  --output "$SCRATCH/iris_landmark.tar.gz" "$SOURCE_URL"
verify_sha "$SCRATCH/iris_landmark.tar.gz" "$SOURCE_TAR_SHA256" "source archive"

echo "Extracting only the expected coreml resource archive..."
listing="$SCRATCH/listing.txt"
tar -tzf "$SCRATCH/iris_landmark.tar.gz" > "$listing"
member="$(grep -m1 -E "(^|/)${INNER_MEMBER}\$" "$listing" || true)"
if [ -z "$member" ]; then
  echo "error: $INNER_MEMBER was not found in the source archive" >&2
  exit 1
fi
tar -xzf "$SCRATCH/iris_landmark.tar.gz" -C "$SCRATCH" "$member"
...
mkdir -p "$MODELS_DIR"
STAGING="$(mktemp -d "$MODELS_DIR/.iris-staging.XXXXXX")"
install_root="$STAGING/iris"
